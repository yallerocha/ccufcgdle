...
export default function ForcaRankingPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true);
        const res = await apiFetch('/api/forca/ranking');
        ...
        console.error('Error loading forca ranking:', err);
  ...
  return (
    <div style={{ margin: '2rem 0' }} className="fade-in">
      <div style={{ marginBottom: '0.5rem' }}>
        <Link href="/forca" className="btn btn-secondary" style={{ fontSize: '0.85rem', padding: '0.4rem 0.8rem', textDecoration: 'none' }}>
            ...
            <table className="admin-table">
              <thead>
                <tr>
                  ...
                  <th style={{ textAlign: 'center' }}>{t('forca.thErrors')}</th>
